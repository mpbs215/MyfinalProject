...
function init(){
	...
	var confmKey = "U01TX0FVVEgyMDE4MDYxMTE3NDA1ODEwNzkzOTg=";//승인키
	var resultType = "4"; // 도로명주소 검색결과 화면 출력유형, 1 : 도로명, 2 : 도로명+지번, 3 : 도로명+상세건물명, 4 : 도로명+지번+상세건물명
	var inputYn= "<%=inputYn%>";
	
	var parameterName = document.form.security.name;
	var token = document.form.security.value;
	...
	if(inputYn != "Y"){
		...
		opener.jusoCallBack("<%=roadFullAddr%>","<%=roadAddrPart1%>","<%=addrDetail%>", "<%=roadAddrPart2%>","<%=engAddr%>"
			...
			, "<%=buldMnnm%>", "<%=buldSlno%>", "<%=mtYn%>", "<%=lnbrMnnm%>", "<%=lnbrSlno%>", "<%=emdNo%>", "<%=entX%>", "<%=entY%>", parameterName, token);
		window.close();
	}
}
</script>
